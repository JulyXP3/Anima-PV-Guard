...
酒馆 → 扩展 面板 → **Anima PV Guard** 里有一组控件：

- **「跳过提示词查看器的 RAG 检索」勾选框** —— 这就是总开关。改完**立刻生效，不需要刷新页面**。
- **「本次强制真实检索」按钮** —— 下次打开提示词查看器时放行一次真实检索，拿一份**按当前聊天重算**的记忆块（会调用一次向量模型），之后自动恢复跳过。想确认"如果现在发消息，模型会看到什么"时用它。
...
AnimaPVGuard.forceLive()       // 等于点「本次强制真实检索」
```
...
1. **新增热开关**：扩展设置面板里多了「跳过提示词查看器的 RAG 检索」勾选框与「本次强制真实检索」按钮，改完**立刻生效、不用刷新页面**（酒馆自带的扩展开关会 `location.reload()`，所以以前每次都得刷新）。开关状态存进酒馆设置（`extension_settings.anima_pv_guard.enabled`），随设置一起保存。
2. **新增状态行**：显示当前开关状态、上一次实际操作（跳过 / 回填 / 放行 / 强制）、快照各条目字数。
3. **新增接口**：`AnimaPVGuard.setEnabled(false)` / `AnimaPVGuard.forceLive()`，方便写卡或脚本里直接切。
4. 「本次强制真实检索」是一次性的：只对下一次打开查看器生效，之后自动恢复跳过。
