...
点「提示词查看器」及右上角刷新图标，期望：浏览器出现 `[Anima PV Guard] 非用户回合（提示词查看器/插件伪造生成），跳过 RAG 检索`，且**没有** `Interceptor Called!`，后端**没有**新的 `Embedding Request`；查看器的提示词列表照常显示。
...
1. 提示词查看器里 `[ANIMA_Chat_History_Container]` 那块会变成空的。Anima 在每次生成结束都会清空该条目，真生成时靠拦截器在拼提示词前重新填入；伪造生成被跳过，于是保持空状态。想看真实检索内容，用 Anima 面板的「最终注入切片 (Merged Results)」按钮。
2. 打开查看器时 Anima 的 `generation_started` 仍会跑（状态倒计时被取消等），这是它自己的监听器，本补丁不碰。

## 可选加强

`index.js` 里的 `SUPPRESS_ANIMA_POST_GEN`（默认 `false`）改为 `true` 后，会在 `CHAT_COMPLETION_SETTINGS_READY` 时刻对伪造生成补发一次 `generation_stopped`，使 Anima 的生成收尾流程（清空注入条目 / 状态更新 / 总结检查）走"生成被中断"的提前返回。代价是向全局事件总线补发合成事件。
